...
def recommend(target_user):
    if target_user in knn_user.keys():
        ...
        print str(target_user)+"Recommendations (np)"+str(to_recommend)
        submission.set_value(target_user, 'recommended_items', ' '.join(to_recommend))
    else:
        print str(target_user)+"Recommendations (tp)"
...
for target_user in target_users.index:
    ...
    t = threading.Thread(target=recommend, args=[target_user])
    threads.append(t)
    t.start()
...
submission.to_csv("../../../data/submission_03_02_04_multithreading.csv", sep=",")
