...
raccomandazioni
"""
target_users = pd.read_csv("../../../data/target_users_2_b.csv", sep="\t", index_col = "user_id")
submission = pd.read_csv("../../../data/target_users_2_b.csv", sep='\t', index_col="user_id")
top_popular = ["2778525","1244196","1386412","278589","657183"]
submission['recommended_items'] = ' '.join(top_popular) # standard top popular

print "-------------------------"
print "Start Recommendations FIRST PART OF TARGET USER 2"
now = datetime.now()
time_start = time.mktime(now.timetuple())
...
for target_user in target_users.index:
    ...
    if target_user in knn_user.keys():
        ...
        sorted_leaderboard = sorted(items_leaderboard, key=items_leaderboard.get, reverse=True)
...
submission.to_csv("../../../data/submission_03_02_04_2b.csv", sep=",")
