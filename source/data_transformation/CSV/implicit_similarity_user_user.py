...
norms_file = "../../../data/processed_data/user_meta.csv"
urm_file = "../../../data/processed_data/user_rating_matrix.csv"

# LAST MOD: aggiunto lo shrink term, per ora definito come la media del numero di interazioni.
...
norms = dict()
for user_id, row in raw_norms.iterrows():
    norms[int(user_id)] = float(row['norm_implicit'])
...
for user_id, interactions in user_grouped:
    ...
    sum_length += len(interactios_for_user[user_id])

shrink_term = sum_length / len(interactios_for_user.keys())
...
def user_similarity(user1, user2):
    try:
        nominator = len(interactios_for_user[user1]
            .intersection(interactios_for_user[user2]))
        denominator = norms[user1] * norms[user2]
        return nominator / (denominator + shrink_term)
    except KeyError:
...
target_users = pd.read_csv("../../../data/raw_data/target_users.csv", sep="\t", index_col = "user_id")
...
with open("../../../data/processed_data/similarity_matrix_user_user_with_shrink.csv", "a") as matrix:
